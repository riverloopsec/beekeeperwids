...
class CapturePlugin(BaseDronePlugin):
	def __init__(self, interfaces, channel, drone):
		...
		self.p_filt.start()
		self.logutil.log(self.desc, 'Launched FilterProcess ({0})'.format(self.p_filt.pid), self.pid)
		...
		self.p_recv.start()
		self.logutil.log(self.desc, 'Launched SnifferProcess: ({0})'.format(self.p_recv.pid), self.pid)
		self.childprocesses.append(self.p_recv)

	def task(self, uuid, data):
		self.logutil.log(self.desc, 'Adding Task: {0}'.format(uuid), self.pid)
	...
	def detask(self, uuid):
		...
		if len(self.tasks) == 0:	
			...
			self.logutil.log(self.desc, 'No remaining tasks, shutting down plugin', self.pid)
			self.shutdown()	
	...
	def __update_filter_tasking(self):
		...
		self.logutil.log(self.desc, 'New Tasking Added', self.pid)
